...
class Terminal(AbstractContextManager["Terminal"]):
    """Small raw terminal wrapper for tui.
    ...
    def __exit__(self, exc_type, exc, tb) -> None:
        ...
        # bracketed paste.  tui does not enable mouse reporting itself, but
    ...
    def _enable_windows_vt(self) -> None:
        ...
        try:
            ...
            if kernel32.GetConsoleMode(input_handle, ctypes.byref(mode)):
                ...
                # Keep ENABLE_PROCESSED_INPUT (0x0001) set. Earlier code cleared
                # it to stop asyncio.to_thread(read_key) from leaking one worker
                # per Ctrl+C, but tui now uses a single
...
class TerminalKeyReader(AbstractContextManager["TerminalKeyReader"]):
    ...
    def _install_sigint_handler(self) -> None:
        ...
        try:
            ...
            # ``signal.signal`` only works in the main thread.  If tui is ever
