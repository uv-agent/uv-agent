...
这些 helpers 由已安装的 uv-agent 插件提供，可在 run_python 中从 uv_agent_runtime 导入。
...
<rule>使用 run_python 时，尽早用 worktree path 调用 rt.cd，以便后续命令在 worktree 中运行。</rule>
...
PROJECT_RULE_INDEX_HEADER = '在活动 {label} 下发现了规则文件。内容已内联于上方任意 <workspace_rules> 块中的文件，视为已加载；不要重新读取。仅对内容未在上方出现的条目使用 rt.cd。'
...
SUBAGENT_LEGACY_UNAVAILABLE = 'legacy ask helper 不可用。请使用 rt.workflow.start(...).agent(...).wait() 或 rt.workflow.agent(...)，然后通过 workflow API 检查 checkpoints/results。'
...
<model_level_policy>
<rule>在节点上传入 model_level，在 rt.workflow.start() 上传入 default_model_level，或两者都省略以使用已配置的 workflow/global 默认值。</rule>
