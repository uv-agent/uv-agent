...
<rule>When a third-party package is needed, install it inside the script with run_process_text(["uv", "pip", "install", "--python", sys.executable, "-q", "..."], check=True). Do not call enter_dir for dependency installation, and do not pass cwd to run_process_text for dependency installation; --python sys.executable selects the target environment.</rule>
<rule>Do not use PEP 723 inline metadata or uv run arguments for dependencies. The run_python tool does not support script_id, run_id, rerun, uv_args, or cwd parameters.</rule>
