...
- **Prompt cache stability (never violate):** The system prompt is the primary provider-side cacheable prefix (Anthropic cache control, OpenAI automatic prefix caching). Its exact byte sequence from position zero must remain identical across calls within the same epoch. Never reorder sections, insert new text, or modify existing text in the stable system prompt. Any change invalidates cached prefixes and wastes user money on cache misses. New model-facing instructions must go into dynamic context blocks—not the system prompt—and must preserve their own intra-epoch prefix stability. The run_python tool definition must also stay consistent between normal calls and compaction calls (both use `[PYTHON_TOOL]`).
