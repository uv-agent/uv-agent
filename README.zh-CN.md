<p align="center">
  <img src="docs/t2.png" alt="uv-agent tui2 截图" width="600">
</p>
...
`uv_agent_runtime` 提供的 helper 完成文件编辑、命令执行、代码搜索、MCP、子 agent、图片
等所有操作。因为只有这一个出口，你可以回放任何一次 run，看清每一步做了什么、为什么这么做。
...
- **Python 托管运行时** — 脚本运行在项目共享的 `uv` 环境中，`uv_agent_runtime` 提供
  read/write/edit、ripgrep 搜索、子进程、依赖安装、子 agent、MCP 客户端等 helper。
  脚本即文档——不像 bash 命令那样晦涩难读。
- **插件体系** — 普通 Python 包通过 `uv_agent.plugins` entry point 发现，可注册
  runtime helper、订阅事件、从外部系统提交 turn。`uvx --with your-plugin uv-agent`
  一行即用。
...
- **Goal mode 持久记忆** — `/goal` 为长任务建立独立于聊天记录的 checklist/notes 层。
  压缩或恢复后，模型优先参考 Goal 文件而非只能依赖摘要后的聊天历史。
- **Agent View 并行工作区** — 把 bug 排查、实现尝试、测试修复等任务派发到隔离 Git
  worktree 的后台会话中并行执行，面板统一跟踪状态。
...
- Agent View 派发后台任务到隔离 Git worktree 并行执行。
- 需要旧版 Textual 面板（`/config`、`/models` 等）时用 `uv-agent tui`。

完整命令见 [TUI and slash commands](docs/tui.md)。

## TUI 界面

- **tui2**（默认，`uv-agent` 或 `uv-agent tui2`）— 轻量 ANSI TUI，直接渲染在终端中。
  紧凑状态行、流式事件、Goal/Worktree mode、图片附件。
- **Textual TUI**（`uv-agent tui`）— 已弃用，保留兼容。基于 Textual widget 的旧界面，
  截图见 [docs/t1.png](docs/t1.png)。

## Agent View

Agent View 是管理多条后台 agent 会话的仪表盘。派发的任务在独立 Git worktree 和自动
生成的分支中运行，与当前 checkout 隔离。面板统一跟踪状态、浏览输出、继续或丢弃任务。

## 插件

插件是通过 `uv_agent.plugins` entry point 发现的普通 Python 包。运行在 uv-agent host
进程中，可以注册 runtime helper、订阅事件，或从外部系统提交 turn。
...
- runtime 上下文（helper 列表、skills、MCP server 等）带指纹增量更新，仅变化部分注入，
  消失的能力显式标记移除。
