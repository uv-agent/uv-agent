...
部分先前可用的运行时上下文已不再存在。对于已移除的 skills 或 MCP servers，除非它们再次出现，否则不要依赖此前追加的内容。
"""
...
COMPACTION_RETURN_ONLY_INSTRUCTION = '只返回延续摘要，使用普通散文文本，不要代码围栏或 tool-call 标记。保留用户意图、决策、文件变更、工具结果和未解决任务。从做了什么、学到了什么的角度总结工具调用；不要复现调用 payload、脚本、JSON、DSML/XML 协议块、stdout 封装或 run IDs。不要复述 AGENTS 目录规则；需要时会自动重新加载。'
