...
<imports>
# 按需导入 helpers；它们不会自动加载，需要从 uv_agent_runtime 显式导入。
...
<usage_pattern>
<rule>helpers 是脚本中使用的普通 Python 函数，不是独立的工具调用；当多个 helpers 服务同一工作单元时，在同一个脚本中导入它们。</rule>
...
<rule>用 Python 方式替代 shell 习惯：适合时用 read_file 代替 cat，用 search_text/find_files 代替临时 grep/find，用 run_process_text([...]) 代替 raw subprocess 或 shell pipelines 来运行普通命令。</rule>
