...
from uv_agent.tui.theme import AnsiTheme, DEFAULT_THEME, sgr
...
def _breath_frame(cell: TranscriptCell) -> int:
    ...
    turn's sliding-window throughput estimate.  Restored scrollback cells and
    focused rendering tests can still use the integer ``chars_streamed`` value.
    """
...
def _tool_cell_import_chains(cell: TranscriptCell) -> list[str]:
    ...
    if isinstance(payload_helpers, list) and payload_helpers:
        # Runtime payloads may include aggregate counts. The terminal UI omits empty
...
def _agent_confirmation_lines(state: TuiState, width: int, theme: AnsiTheme) -> list[str]:
    view = state.agent_view
...
def _agent_peek_lines(state: TuiState, width: int, theme: AnsiTheme) -> list[str]:
    view = state.agent_view
    lang = _resolve_language(state.language)
    selected = view.selected_row()
    lines: list[str] = []
...
def render_live(state: TuiState, width: int, spinner_frame: int = 0) -> list[str]:
