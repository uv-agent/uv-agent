...
class WorkflowExecutor:
    ...
    async def _execute_node(self, workflow: dict[str, Any], node: dict[str, Any]) -> None:
        ...
        try:
            handle = await self.turn_manager.submit_turn(
                user_text=str(node.get("prompt") or ""),
                thread_id=thread_id,
                level=level,
                conflict="queue",
            )
            ...
            if handle.status == "completed":
                ...
                status = "failed"
                error = {"type": str(handle.status), "message": str(handle.error or handle.status)}
            result = {"thread_id": thread_id, "turn_id": handle.turn_id, "model_level": level, "status": handle.status}
            with connect_state_db(self.data_dir) as db:
                db.execute(
                    """
                    UPDATE workflow_nodes
                    ...
                    """,
                    (status, completed, thread_id, handle.final_text, _dumps(result), _dumps(error), node_id, self.executor_id),
                )
    ...
    def _create_node_thread(self, workflow: dict[str, Any], node: dict[str, Any]) -> str:
        ...
        thread_id = self.thread_store.create_thread(
        ...
        self.thread_store.append(thread_id, "thread.workflow_node_bound", workflow_id=workflow["workflow_id"], node_id=node["node_id"])
        with connect_state_db(self.data_dir) as db:
