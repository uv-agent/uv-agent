...
`AGENTS.md`，启动时会按用户语言创建一份结构化的中文或英文说明。可以用
`--workspace <path>` 指定其他 daemon 工作区。
...
插件较多时，可以用独立发布的 `uv-agentx` 启动器保持 uvx 临时环境，同时缩短命令：

```powershell
uvx uv-agentx@latest --latest -p auth-code -p remote-control -- daemon --replace
```

`-p auth-code` 会先尝试官方 PyPI 包 `uv-agent-auth-code`，不存在时再回退到
`auth-code`。需要完全原样传入包名时使用 `--raw-plugin`。完整语法见
[uv-agentx](packages/uv-agentx/README.md)。
