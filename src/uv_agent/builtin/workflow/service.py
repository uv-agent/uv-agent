...
from uv_agent_runtime.events import emit_event
    ...
    *,
    ...
    with _connect(base) as db:
        db.execute(
            ...
            (
                workflow_id,
                parent_thread_id if parent_thread_id is not None else os.environ.get("UV_AGENT_RUNTIME_THREAD_ID"),
                parent_turn_id if parent_turn_id is not None else os.environ.get("UV_AGENT_RUNTIME_TURN_ID"),
                parent_run_id if parent_run_id is not None else os.environ.get("UV_AGENT_RUNTIME_RUN_ID"),
                objective,
                default_model_level,
...
def _connect(base: Path) -> sqlite3.Connection:
    connection = connect_state_db(base)
    try:
...
def connect_workflow_db(data_dir: Path) -> sqlite3.Connection:
